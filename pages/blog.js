...
const page = {
  title : 'Blog',
  desc : 'Catch up on all the F2K blog posts here.',
  canonical: 'https://f2k.gg/blog'
}
...
export default function Blog() {
    ...
    else
    {
        ...
        const { data, error } = useSWR( host + '/backend/get_posts.php?allPosts=true&post_type=blog' ,fetcher(host + '/backend/get_posts.php?allPosts=true&post_type=blog'))

        if(error)
            return (
                <Layout page = {page} scrolled = {'disabled'}>
                    <div className={styles.title}>
                        <Title title = {'f2k blog'} />
                    </div>
                    <div className = 'posts_wrapper container'>
                        <h1>Error loading posts</h1>
                    </div>
            ...
            <Layout page = {page} scrolled = {'disabled'}>
                ...
                <div className = 'posts_wrapper container'>
                    ...
                    {data ? data.sort((a,b) => new Date(b.created_at) - new Date(a.created_at)).map((post, index) => 
                            ...
                            <Link key={index} href={`/blog?slug=${post.slug}`} >
                                <a>
